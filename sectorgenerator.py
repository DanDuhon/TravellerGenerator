...
def sectorgen(
    ...
    openClusterBorder = math.floor(
        initialSectorSize * openClusterPercent / 100) - sectorMax
    hexes = {}
    ...
    for h in range(sectorMin, sectorMax + 1):
        for v in range(sectorMin, sectorMax + 1):
            ...
            hexes.update({(h, v): systemhex.System(
    ...
    while True:
        while True:
            ...
            # Increase settlement, which tracks how long a planet has been
            # colonized.
            for p in [p for p in planet.allPlanets if set(
                    ["Colony", "Outpost"]) <= set(p.habitation.values())]:
                p.settlement += 1

            # Reassess the already colonized planets to see if anything has
            # changed.
            for a in [
                    a for a in alien.allAliens if not a.extinct and a.currentTechLevel >= 9]:
                ...
                # Reminder to check the comments in the habitation method when
                # terraforming is implemented.
                ...
                for s in set([i.systemHex for i in colonizedPlanets]):
                    ...
                    if a.currentTechLevel == 9:
                        systemsExplored.add(
                            (s.horizontalCoord, s.verticalCoord, s.cubeCoord))
                    else:
                        for x in range(3 + a.reactionModifier + (
                                1 if any([True for p in s.planets if p.habitation[a] == "Colony"]) else 0)):
                ...
                for s in systemsExplored:
                    if (s[0], s[1], s[2]) not in systemhex.allCoordinates:
                        hexes.update({(s[0], s[1]): systemhex.System(
                            s[0], s[1], openClusterBonus, alienSurvivalPercent, maxTechLevel)})
                    planetsExplored.update(
                        [(a, p) for p in systemhex.allCoordinates[s].planets if a not in p.desirability.keys() or p.habitation[a] is None])
        ...
        if max(
                [a.currentTechLevel for a in alien.allAliens if not a.extinct]) == maxTechLevel:
            break
